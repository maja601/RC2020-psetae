...
class PixelSetData(data.Dataset):
    """
    from https://github.com/VSainteuf/pytorch-psetae/blob/master/dataset.py
    """
    ...
    def __getitem__(self, item):
        ...
        if self.extra_feature is not None:
            ...
            ef = torch.stack([ef for _ in range(x.shape[0])], dim=0)
            # data = (data, ef)
        ...
        sample = {'data': Tensor(x),
                  'label': torch.from_numpy(np.array(y, dtype=int)),
                  'geom': ef,
                  'mask': Tensor(mask)}
        return sample
